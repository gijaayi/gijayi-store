  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setMessage('If an account exists, a password reset link was sent to the provided email.');
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-xl shadow-xl overflow-hidden border border-slate-200">
          <div className="px-8 py-10 border-b border-slate-200 bg-linear-to-r from-slate-50 to-white">
            <p className="text-xs tracking-[0.35em] uppercase text-slate-500 font-medium mb-2">Password Help</p>
            <h1 className="font-serif text-2xl text-slate-900">Reset Password</h1>
          </div>

          <div className="px-8 py-8">
            <p className="text-sm text-slate-600 mb-6">Enter your account email and we'll send reset instructions.</p>
